...
extends OperatorBase
{
    ...
    constructor(options = {})
    {
        super(VAR_NUMBER, 'number', 'number', '');
        ...
        this.varIcon      = createDiv('styleCircleWrapper');;
        this.varIcon.over = false;

        this.varIcon.addEventListener('pointerenter', e => { this.varIcon.over = true;  this.updateLinkIcon(); });
        this.varIcon.addEventListener('pointerleave', e => { this.varIcon.over = false; this.updateLinkIcon(); });

        this.varIcon.addEventListener('pointerdown',  e => 
        { 
            e.stopPropagation();
        ...
        this.varIcon.appendChild(this.circleBack);
        this.varIcon.appendChild(this.circleCheckers);
        this.varIcon.appendChild(this.circle);
        this.varIcon.appendChild(this.link);
        
        this.label.insertBefore(this.varIcon, this.labelText);
    ...
    updateHeader()
    {
        ...
        else
        {
            ...
            this.varIcon   .style.boxShadow  = '0 0 0 1px var(--figma-color-bg-tertiary) inset';
        }
        ...
        if (this.existing)
        {
            ...
            const linkStyle = rgba2style(
                rgb_a(
                    ...
                    : colors.text, 
                    this.linkedStyleId != NULL
                    ? (this.varIcon.over ? 0.5 : 0)
                    : (this.varIcon.over ? 1 : 0.5)));
