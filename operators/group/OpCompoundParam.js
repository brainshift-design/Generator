class   OpCompoundParam
extends OperatorBase
{
    static { Operator.types[COMPOUND_PARAM] = this; }
    ...
    get compoundNode() 
    { 
        return graph.currentPage.compoundId != NULL
             ? nodeFromId(graph.currentPage.compoundId)
             : null;
    }
    ...
    constructor()
    {
        super(COMPOUND_PARAM, 'param', 'parameter', '');
    ...
    setName(newName, options = {})
    {
        if (this.paramType == 0)
        {
            ...
            if (this.groupParam)
            {
                //const param = this.compoundNode.paramFromId(this.name);
            ...
            if (this.compoundNode)
            {
                const id = makeNodePath(this);
                const compoundId = makeNodePath(this.compoundNode.id);


                uiSaveNodes([id]);
                uiSaveNodes([compoundId]);
                
                uiUpdateSavedConnectionsToNodeId  ([id], true);
                uiUpdateSavedConnectionsFromNodeId([id], true);
                
                uiUpdateSavedConnectionsToNodeId  ([compoundId], true);
                uiUpdateSavedConnectionsFromNodeId([compoundId], true);


                nodeFromId(compoundId).updateParams();
            }
        }
    ...
    setPosition(x, y, updateTransform = true)
    {
        ...
        if (this.compoundNode)
        {
            this.compoundNode.updateNode();
            graphView.updateNodeWireTransforms([this.compoundNode]);
        }
    }
...
function input_onconnect(node)
{
    ...
    if (node.paramType == 0)
    {
        ...
        node.compoundNode.addParam(node.groupParam);
    }
    else if (node.paramType == 1)
    {
        node.groupOutput = new Output([...node.inputs[0].types], node.compoundNode.output_genRequest);
        node.compoundNode.addOutput(node.groupOutput);
    }
    

    node.groupOutput.paramNode = node;
    node.compoundNode.updateNode();
}
...
function input_ondisconnect(node)
{
    ...
    if (node.paramType == 0)
    {
        node.compoundNode.removeParam(node.groupParam);
        node.groupOutput.paramNode = null;
    }
    else if (node.paramType == 1)
    {
        node.compoundNode.removeOutput(node.groupOutput);
        node.groupOutput.paramNode = null;
    }
...
function output_onconnect(node)
{
    ...
    else if (node.paramType == 1)
    {
        ...
        node.compoundNode.addInput(node.groupInput);
    }
...
function output_ondisconnect(node)
{
    ...
    if (node.paramType == 0) 
    {
        ...
        node.compoundNode.removeInput(node.groupInput);
