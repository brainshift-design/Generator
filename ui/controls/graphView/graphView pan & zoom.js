...
graphView.endZoomSelection = (pointerId, zoom) =>
{
    ...
    if (zoom)
    {
        ...
        const diff = { w: (window.innerWidth / graphView.zoom - box.w) / 2,
                       h: (wndHeight         / graphView.zoom - box.h) / 2 };

        console.log('box.x = ' + box.x + ', box.y = ' + box.y + ', box.w = ' + box.w + ', box.h = ' + box.h + ', diff.w = ' + diff.w + ', diff.h = ' + diff.h + ', wnd.width = ' + window.innerWidth + ', wnd.height = ' + wndHeight + ', zoom = ' + graphView.zoom);
