        ...
        static parse(str, i = -1)
        {
            ...
            for (let j = 0; j < nInputs; j++)
            {
                ...
                switch (type)
                {
                    ...
                    case     RECTANGLE_VALUE: { const rect    = RectangleValue   .parse(str, i);  i += rect   [1];  group.items.push(rect   [0]);  break; }
                    case          LINE_VALUE: { const line    = LineValue        .parse(str, i);  i += line   [1];  group.items.push(line   [0]);  break; }
                    case       ELLIPSE_VALUE: { const ellipse = EllipseValue     .parse(str, i);  i += ellipse[1];  group.items.push(ellipse[0]);  break; }
                    case       TRAPEZE_VALUE: { const ellipse = TrapezeValue     .parse(str, i);  i += ellipse[1];  group.items.push(ellipse[0]);  break; }
                    case       POLYGON_VALUE: { const poly    = PolygonValue     .parse(str, i);  i += poly   [1];  group.items.push(poly   [0]);  break; }
                    case          STAR_VALUE: { const star    = StarValue        .parse(str, i);  i += star   [1];  group.items.push(star   [0]);  break; }
                    case    TEXT_SHAPE_VALUE: { const text    = TextShapeValue   .parse(str, i);  i += text   [1];  group.items.push(text   [0]);  break; }
                    ...
                    case SHAPE_BOOLEAN_VALUE: { const path    = ShapeBooleanValue.parse(str, i);  i += path   [1];  group.items.push(path   [0]);  break; }
                    case   SHAPE_GROUP_VALUE: { const _group  = ShapeGroupValue  .parse(str, i);  i += _group [1];  group.items.push(_group [0]);  break; }
                    case         FRAME_VALUE: { const frame   = FrameValue       .parse(str, i);  i += frame  [1];  group.items.push(frame  [0]);  break; }
                }
            }
